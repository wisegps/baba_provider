/**
 * 二维码扫描工具，在本地应用使用的是html5+提供的工具；
 * 在微信端调用的是微信的jssdk（即微信原生扫描），因为调用jssdk需要引入微信的js文件，以及配置相关信息，
 * 所以并不能马上使用，而需等待window的scannerReady事件，如果是用户交互而调用，则可以通过判断W.scanner是否存在来确定是否可以使用；
 * (微信jssdk需要在指定域名下使用，所以目前以下代码未进行测试)
 */
		...
		onload:function(){//加载完js之后，开始配置jssdk
			if(W.wx.ticket){//如果有ticket,则开始配置微信sdk
				...
				wx.config({
				    ...
				    jsApiList: ["scanQRCode"] // 需要使用的js接口
				});
				wx.ready(function(){
					W.scanner={
						//补充对象方法
